...
### API 리버스 엔지니어링을 위한 에이전트 네이티브 워크벤치.
...
- **WASM 분석** — 캡처된 `.wasm` 모듈을 읽을 수 있는 텍스트로 디스어셈블합니다 — WAT(항상 사용 가능한 인프로세스 baseline), C 유사 뷰, 또는 완전한 C 소스(`wasm2c`). export와 심볼 이름을 요약하고, body에서 임베디드 문자열·키를 스캔하며, 각 export를 그것을 호출하는 JS와 교차참조합니다. 요청 서명이 WebAssembly로 컴파일되어 Web Crypto 훅이 아무것도 보지 못하는 사각지대를 메웁니다. 트래픽 저장소에 이미 있는 `.wasm` 바이트를 읽고(재다운로드 없음), 캡처 수정으로 캐시·서비스 워커로 전달된 모듈도 온전히 보존합니다.
...
- **암호화 & 서명 리버싱** — Web Crypto를 실시간 후킹해 HMAC 키, 서명 대상 메시지, 그리고 결과 서명을 호출 시점에 캡처하고(디버거 불필요), HMAC/해시를 계산·반복하며, 중첩된 base64/hex/url/rot13 디코딩 체인을 풀고, JWT를 비롯한 토큰을 디코딩하며, PHP "매직 해시"를 조회하고, `.proto` 없이 불투명한 protobuf / gRPC-web 본문을 디코딩합니다.
- **구조 탐색** — GraphQL 스키마 인트로스펙션(SDL 변환), 요청에서 그 요청을 발생시킨 `script:line`으로 바로 이동하는 이니시에이터 콜 스택, API 베이스 탐지, 요청 diff, WebSocket 프레임 캡처, 서비스 워커 / Cache API 검사를 제공합니다.
- **보안 점검** — 캡처된 트래픽에서 임베디드 시크릿(JWT, 클라우드/API 키, 개인 키)을 스윕하고, 보안 헤더를 분석하며, 리플렉션/XSS·경로 탐색/LFI·CRLF 프로브를 실행합니다 — 내보낼 수 있는 findings 로그와 함께.
- **내보내기** — HAR 1.2(Burp Suite, Caido 등 HAR 분석기로 로드 가능)와, 캡처된 요청을 재현하는 바로 실행 가능한 Python(`requests` / `httpx`) 클라이언트를 생성합니다.
- **비전 판단(Vision judge)** — 접근성 트리로는 알 수 없는 상황(캡차, 모달 오버레이, 검색이 실제로 실행됐는지 등)에서 에이전트가 페이지를 스크린샷으로 찍어 비전 모델에게 화면에 실제로 보이는 것을 판단하게 합니다.

- **매크로 & 워크플로** — 일련의 도구 호출을 LLM 없이 결정론적으로 재생되는 이름 있는 매크로로 만듭니다. 앱 내장 Macro·Pipeline·Template 에디터로 이런 원클릭 흐름을 구성합니다.
