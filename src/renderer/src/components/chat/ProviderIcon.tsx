...
// Both `claude-code` and `anthropic` are Claude surfaces → Anthropic mark;
// both `codex` and `openai` are OpenAI surfaces → OpenAI mark.

const ANTHROPIC_PATH =
  'M13.827 3.52h3.603L24 20.48h-3.603l-6.57-16.96zm-7.258 0h3.767l6.57 16.96H13.6l-1.343-3.463H5.017l-1.344 3.463H0L6.57 3.52zm4.132 10.226L8.453 7.4l-2.248 6.346h4.496z'
...
// Anthropic's clay is on-brand and readable on both themes; OpenAI's mark uses
// the current text color so it flips black/white with the theme.
const MARK: Record<ACPAgentID, { path: string; color: string } | undefined> = {
  'claude-code': { path: ANTHROPIC_PATH, color: '#d97757' },
  anthropic: { path: ANTHROPIC_PATH, color: '#d97757' },
  codex: { path: OPENAI_PATH, color: 'currentColor' },
  openai: { path: OPENAI_PATH, color: 'currentColor' }
}
